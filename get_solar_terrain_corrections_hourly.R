...
outTR = params$outTR[1,1] # temporal resolution (hrs)
deltat = 1; # run the code hourly
...
solar_mtn <- array(rep(NaN, nsites*12*(24/deltat)), c(nsites, 12, (24/deltat)));   # site, month, hour
...
for (mm in 1:12){
  ...
  for (tt in 1:length(ts)){ # for each time interval b/n sunrise and sunset:
    ...
    if (srs<=day1[1] | srs>=day1[2]){ # if hr is during dark, no terrain correction
      ...
      # Also save the fine res solar values to use for aggregating the ratios
      Irr <- projectRaster(Irr,demorig)
      Irr <- raster::extract(Irr,pts_to_model)
      solar_mtn[,mm,tt] <- Irr
...
# then aggregate ratios based on solar_mtn to the desired output temporal resolution
# when there are no nighttime timesteps:
#outTR = 4
nt = 24/outTR
stts = seq(1,24,outTR)
ents = seq(4,24,outTR)
...
for (ss in 1:size(solar_tc,1)){
  for (mm in 1:12){
    for (ttt in 1:nt){
      st <- stts[ttt]
      en <- ents[ttt]
      newtc[ss,mm,ttt] <- sum(solar_tc[ss,mm,st:en]*solar_mtn[ss,mm,st:en])/sum(solar_mtn[ss,mm,st:en])
      if (sum(solar_mtn[ss,mm,st:en])==0) {
        newtc[ss,mm,ttt] <- 1
      }
    }
  }
}
#writeMat('/Volumes/WDPassport/DATA/WRF/Downscaled/solartc_CTRL_210m_chunk45.mat', solar_tc = newtc)
#newtc <- sum(solar_tc[1000,1,20:23]*solar_mtn[1000,1,20:23])/sum(solar_mtn[1000,1,20:23])
# when there are nighttime timesteps:
#mean(solar_tc[70507,1,1:4])


# save terrain corrections
writeMat(outfn, solar_tc = newtc)
