function[] = pick_modeling_locations(ch, outDEMf, outDEMc, outSRf, outSRc, elev_dif_thres, aspect_dif_thres, outdir)
...
outelevf = outlonc;
    ...
    for cc = 1:size(elevco,1)
        ...
        %[FX, FY] = gradient(fine_elevs);
        %figure(4);clf;imagesc(FX);colorbar();
        %figure(5);clf;imagesc(FY);colorbar();
        
        %gr = sqrt(FX.^2 + FY.^2);
        %figure(6);clf;imagesc((gr));colorbar();
        %aspectdif = ;
        
        if (elevdif < elev_dif_thres) % && aspectdif < aspect_dif_thres)
            % append lon/lat of coarse grid cell to list of locations to
            % model
